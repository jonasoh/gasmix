...
def plot_flow(num, color):
    return (
        f"DEF:flow={RRD_FILES[num]}:flow:AVERAGE",
...
def plot_h2(num, color):
    return f"DEF:h2={RRD_FILES[num]}:h2:AVERAGE", f"LINE2:h2#{color}:H2 (%)"


def plot_co2(num, color):
    return f"DEF:co2={RRD_FILES[num]}:co2:AVERAGE", f"LINE2:co2#{color}:CO2 (%)"
